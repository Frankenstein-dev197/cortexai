 ...
 * For example, if the tools requires `npx` then the context in which Cortex AI mains process is running will need to access npx.
 * This is typically not common in our pre-built image so may not function. But this is the case anywhere MCP is used.
 *
 * Cortex AI will take care of porting MCP servers to agent-callable functions via @agent directive.
