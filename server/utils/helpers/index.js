...
function getLLMProvider({ provider = null, model = null } = {}) {
  ...
  switch (LLMSelection) {
    ...
    case "anythingllm-router":
      // Model router is handled separately in stream.js via CortexModelRouter.
      // This case should not be hit directly - if it is, throw a descriptive error.
      throw new Error(
        "anythingllm-router provider must be resolved via CortexModelRouter class, not getLLMProvider directly."
      );
    default:
      throw new Error(
...
function getLLMProviderClass({ provider = null } = {}) {
  switch (provider) {
    ...
    case "cerebras":
      ...
      return CerebrasLLM;
    case "anythingllm-router":
      const { CortexModelRouter } = require("../AiProviders/modelRouter");
      return CortexModelRouter;
    default:
      return null;
  }
}
...
async function resolveProviderConnector({
  ...
  const router = new CortexModelRouter(routerWorkspace);
