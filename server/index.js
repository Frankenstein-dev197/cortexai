...
if (process.env.NODE_ENV === "development" && !!process.env.ENABLE_HTTP_LOGGER) {
  app.use(httpLogger({ enableTimestamps: !!process.env.ENABLE_HTTP_LOGGER_TIMESTAMPS }));
}
app.use(cors({ origin: true }));
// Stripe signs the exact raw request body. This must run before JSON parsing.
  ...
  apiRouter.post("/v/:command", async (request, response) => {
    try {
      ...
      if (!Object.getOwnPropertyNames(VectorDb).includes(command)) {
        response.status(500).json({ message: "invalid interface command", commands: Object.getOwnPropertyNames(VectorDb) });
        return;
      }
      try {
